---
name: ralph-loop
description: Interactive coding loop with per-task Claude TUI sessions. Each task runs in auto mode — you can attach to any tmux window to watch, interact, or redirect the agent mid-task. Includes per-task summaries and session resumability. Use when asked to "set up a ralph loop", "interactive ralph loop", "coding loop", or when you want visibility and the ability to intervene. For fully headless execution, use ralph-loop-auto instead.
---

# Ralph Loop (Interactive)

Autonomous coding loop with interactive Claude TUI sessions. Each task runs in its own tmux window with `--permission-mode auto` — attach to watch, interact, or redirect the agent mid-task. After each task, a summary is generated and a resume command is printed. A watchdog script monitors progress, handles escalations, and restarts the runner if it dies.

**For fully headless, fire-and-forget execution**, use `ralph-loop-auto` instead — it uses `claude -p` (print mode) with no permission classifier, which is more reliable but not interactive.
